...
export function ModeExplainer() {
  ...
  useGsapReveal(modesRef, { delay: 0.4, stagger: 0.2 });

  const icons = [Zap, Shield, Settings];
    ...
    <section ref={containerRef} className="py-24 bg-zinc-950/50 relative overflow-hidden">
      <div className="container mx-auto px-4">
        <SectionHeading
          ...
          description="Understanding the different ways to publish your content."
          align="center"
          className="mb-16"
        />

        <div ref={modesRef} className="grid md:grid-cols-3 gap-8">
          {Object.entries(CONTENT.platforms.modes).map(([key, value], i) => {
            const Icon = icons[i];
            return (
